...
export default function SideBar() {
  ...
  return (
  <aside className="sticky hidden sm:block w-20 bg-zinc-900 p-4 min-h-screen text-white">
  <nav className="flex flex-col items-center justi fy-center gap-5 h-11/12">
    {categories.map((item) => (
      <Link
        key={item.name}
        to={item.path}
